...
\begin{response}
This is an excellent point, and is completely true! The text has now been revised to emphasize that increasing deep-sequencing counts stop being meaningful once the typical number of counts per site becomes on the order of the number of unique molecules that were originally in the experiment. For most shotgun sequencing experiments, the typical counts per site ($\sim 10^5$ to $10^6$) still tend to be less than the typical number of unique molecules ($\sim 10^6$ to $10^8$), but this will not always be true particularly as sequencing depths increase.

The revised text now emphasizes this point. It emphasizes that the experiments should be done in a way to ensure that the sequencing depth is not saturating the number of unique molecules (i.e. by quantifying the number of molecules carefully). It also suggests that one way that this issue can be examined during \textit{post hoc} computational analyses is by sub-sampling the deep sequencing data (i.e. randomly selecting only a portion of the sequencing reads) when generated the mutation counts files used as input to inference programs in \textsl{dms\_tools}, and then seeing if this reduced depth alters the results. When the results start to become insensitive to sub-sampling the data, this provides an indication that the unique molecules are starting to be oversampled. 
\end{response}
