...
def BarcodedSubampliconsParser():
    ...
    parser.add_argument('--purgereadfrac', type=FloatBetweenZeroAndOne, default=0, help='Randomly purge read pairs with this probability, thereby subsampling the data. You might want a value > 0 to estimate how the results depend on the sequencing depth. This argument differs from "--purgefrac", which purges barcodes instead of read pairs. In deciding which option to use, it is important to keep in mind that the number of unique barcodes sequenced does not necessarily increase linearly with read depth. It is not recommended that both of these arguments are used at the same time.')
    parser.add_argument('--purgefrac', type=FloatBetweenZeroAndOne, default=0, help='Randomly purge barcodes with this probability, thereby subsampling the data. You might want a value > 0 to estimate how the results depend on the number of barcodes. This argument differs from "--purgereadfrac", which purges read pairs instead of barcodes. In deciding which option to use, it is important to keep in mind that the number of unique barcodes sequenced does not necessarily increase linearly with read depth. It is not recommended that both of these arguments are used at the same time.')
